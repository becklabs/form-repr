...
from src.data.dataset import PoseTrackDataset2D
...
def train_with_config(args, opts):
    ...
    trainloader_params = {
          ...
          'shuffle': True,
          'num_workers': 2, # min(12, os.cpu_count()),
          'pin_memory': True,
          'prefetch_factor': 4,
    ...
    if args.train_2d:
        posetrack = PoseTrackDataset2D(data_root='./data/poses/All_Data/')
    ...
    if not opts.evaluate:        
        ...
        st = 0
        if args.train_2d:
            print('INFO: Training on {}(3D)+{}(2D) batches'.format(len(train_loader_3d), len(posetrack_loader_2d)))
        else:
