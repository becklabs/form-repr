...
def read_input(json_path, vid_size, scale_range, focus):
    ...
    kpts_all = np.array([frame['predictions_2d'][0]['keypoints'] for frame in results])
    keypoint_scores = np.array([frame['predictions_2d'][0]['keypoint_scores'] for frame in results])
    # print(kpts_all.shape)
    # print(keypoint_scores[:, :, np.newaxis].shape)
    kpts_all = np.dstack((kpts_all, keypoint_scores[:, :, np.newaxis]))
